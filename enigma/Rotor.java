...
class Rotor {
    ...
    int convertForward(int p) {
        int result = _permutation.wrap(
                _permutation.permute(_permutation.wrap(
                        p + _setting - _ring)) - _setting + _ring);
        if (Main.verbose()) {
    ...
    int convertBackward(int e) {
        int result = _permutation.wrap(
                _permutation.invert(_permutation.wrap(
                        e + _setting - _ring)) - _setting + _ring);
        if (Main.verbose()) {
            System.err.printf("%c -> ", alphabet().toChar(result));
        }
        return result;
    ...
    boolean atNotch() {
        for (int i = 0; i < notches().length(); i += 1) {
            if (alphabet().alpha().charAt(_setting) == (notches().charAt(i))) {
                return true;
            }
        }
        return false;
